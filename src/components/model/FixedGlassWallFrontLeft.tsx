...
export default function FixedGlassWallFrontLeft({
  nodes,
  materials,
  metalTexture,
}: Props) {
  ...
  const getMaterial = () => {
    switch (frontLeftMaterialTransparency) {
      case "clear":
      ...
      {width >= 607 && (
        <>
          {frontLeftWallType === "fixedGlassWall" && (
            <group name="fixedGlassWallFrontLeft">
              <mesh
              ...
              <mesh
                ...
                position={[
                  -1.365 * frameScaleD,
                  0.807 * frameScaleH,
                  -0.07 * frameScaleW + (width > 400 ? 0.17 : 0.07),
                ]}
                rotation={[0, 0, -Math.PI]}
                scale={[-1, -1, -1.011]}
                scale-y={frameScaleH}
